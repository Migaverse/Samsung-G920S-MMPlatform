	...
	if (success)
	{
		...
		switch(process_id)
	  	...
	  	case 0: /* child */
			...
			snprintf(remote_addr, sizeof(remote_addr), "%H", this->ike_sa->get_other_host(this->ike_sa));
			...
			err |= setenv("REMOTE_ADDR", remote_addr, TRUE);
			if (err == 0)
			{
	   ...
	   private_android_service_t *this, ike_sa_t *ike_sa, bool up)
{
    ...
    char gateway_ip[256];
    memset(gateway_ip, 0, 256);
    sprintf(gateway_ip, "%H", ike_sa->get_other_host(ike_sa));
    DBG1(DBG_KNL, "[SEC_UNITY_SPLIT_INCLUDE] ike_updown: gateway = %s", gateway_ip);
    lib->settings->set_str(lib->settings, "charon.gateway", gateway_ip);
	//SEC_UNITY_SPLIT_INCLUDE ]
	return TRUE;
}

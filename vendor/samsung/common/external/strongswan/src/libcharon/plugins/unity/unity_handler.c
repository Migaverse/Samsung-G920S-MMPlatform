...
//SEC_UNITY_SPLIT_INCLUDE [
bool add_ts_for_unity(void *ts_void, bool type)
{
	traffic_selector_t *ts = (traffic_selector_t *)ts_void;
	ifaces_activated = lib->settings->get_str(lib->settings, "charon.interface_used", NULL);
	if(!ifaces_activated)
	{
		return FALSE;
	}
	...
	if(type)
	{
		opt = "charon.include_routing_tables";
	}
	else
	{
		opt = "charon.exclude_routing_tables";
	}

	size_t old_ts_len = 0;
	char cur_ts[128] = {0,};
	char *old_ts = lib->settings->get_str(lib->settings, opt, NULL);
	if(old_ts)
		old_ts_len = strlen(old_ts);

	char *new_ts = malloc(old_ts_len+128);
	memset(new_ts, 0, old_ts_len+128);
	memcpy(new_ts, old_ts, old_ts_len);
	sprintf(cur_ts, "%R", ts);

	if(old_ts)
		strcat(new_ts, " ");
	strcat(new_ts, cur_ts);
	lib->settings->set_str(lib->settings, opt, new_ts);
	
	DBG2(DBG_KNL, "[UNITY_SPLIT_INCLUDE_EXCLUDE] add_route : %s = %s", opt, new_ts);
    free(new_ts);
	return TRUE;
}
//SEC_UNITY_SPLIT_INCLUDE ]
...
static job_requeue_t add_exclude_async(entry_t *entry)
{
	...
	if (ike_sa)
	{
		...
		DBG2(DBG_IKE, "installed %N bypass policy for %R",
...
static bool add_exclude(private_unity_handler_t *this, chunk_t data)
{
	...
	list->destroy(list);
	//SEC_UNITY_SPLIT_INCLUDE [
	DBG2(DBG_IKE, "[UNITY_SPLIT_INCLUDE] add_exclude TS = %R", ts);
	return add_ts_for_unity(ts, FALSE); // FALSE for exclude
	//return TRUE;
	//SEC_UNITY_SPLIT_INCLUDE ]
}
...
static bool remove_exclude(private_unity_handler_t *this, chunk_t data)
{
	...
	while (list->remove_first(list, (void**)&ts) == SUCCESS)
	{
		...
		DBG2(DBG_IKE, "uninstalling %N bypass policy for %R",
	...
	configuration_attribute_type_t type, chunk_t data)
{
	...
	//SEC_UNITY_SPLIT_INCLUDE [
	lib->settings->set_str(lib->settings, "charon.interface_used", NULL);
	lib->settings->set_str(lib->settings, "charon.include_routing_tables", NULL);
	lib->settings->set_str(lib->settings, "charon.exclude_routing_tables", NULL);
	lib->settings->set_bool(lib->settings, "charon.is_ts_include_installed", FALSE);
	lib->settings->set_bool(lib->settings, "charon.is_ts_exclude_installed", FALSE);
	DBG2(DBG_KNL, "[UNITY_SPLIT_INCLUDE] reset unity split settings");
	//SEC_UNITY_SPLIT_INCLUDE ]
}

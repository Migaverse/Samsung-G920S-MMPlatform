...
int read_arguments(private_android_config_t *this, int fd)
{
  ...
  switch(this->public.auth_method)
    ...
    case AUTH_XAUTH_INIT_RSA:
      ...
      if (err != 0) goto error;
      err = read_argument_string(fd, &(this->public.rsa.cc_mode));
      if (err != 0) goto error;
      err = read_argument_string(fd, &(this->public.rsa.is_pfs));
      if (err != 0) goto error;
      err = read_xauth_arguments(this, fd);
      if (err != 0) goto error;
      

      lib->settings->set_str(lib->settings, "%s.cc_mode",
						   this->public.rsa.cc_mode, lib->ns);
      break;
      ...
      else
	{
      ...
      err = read_argument_string(fd, &(this->public.psk.cc_mode));
      if (err != 0) goto error;
      
      err = read_argument_string(fd, &(this->public.psk.is_pfs));
      if (err != 0) goto error;
	   ...
	   private_android_config_t *this)
{
  ...
  if (this->public.auth_method == AUTH_XAUTH_INIT_RSA ||
      this->public.auth_method == AUTH_RSA)
    {
      ...
      FREE_IF(this->public.rsa.cc_mode);
    }
           ...
           this->public.auth_method == AUTH_PSK)
    {
      ...
      FREE_IF(this->public.psk.cc_mode);
    }

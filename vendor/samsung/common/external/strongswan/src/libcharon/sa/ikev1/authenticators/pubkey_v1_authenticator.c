...
#define MAX_ITER_FOR_FINDING_KEY 2
	...
	private_pubkey_v1_authenticator_t *this, message_t *message)
{
    ...
    size_t count = 0;
    while(count++ < MAX_ITER_FOR_FINDING_KEY)
    {
	...
	if (!private)
	{
		DBG1(DBG_IKE, "no %N private key found for '%Y'",
            ...
            if(count < MAX_ITER_FOR_FINDING_KEY)
            {
                DBG1(DBG_IKE, "Trying to find ECDSA private key");
                this->type = KEY_ECDSA;
                continue;
            }
		return NOT_FOUND;
	...
	private_pubkey_v1_authenticator_t *this, message_t *message)
{
	...
	if (status != SUCCESS)
	{
		DBG1(DBG_IKE, "no trusted %N public key found for '%Y'",
            ...
            if(count < MAX_ITER_FOR_FINDING_KEY)
            {
                DBG1(DBG_IKE, "Trying to find trusted ECDSA public key");
                this->type = KEY_ECDSA;
            }
	    }
	}

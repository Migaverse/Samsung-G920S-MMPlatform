	...
	private_main_mode_t *this, message_t *message)
{
	switch (this->state)
	{
		case MM_INIT:
		{
			...
			this->auth_peer = this->ph1->auth_peer(this->ph1, this->peer_cfg);
	...
	private_main_mode_t *this, message_t *message)
{
	switch (this->state)
	{
		...
		case MM_AUTH:
		{
            ...
            char *cc_mode;         
            cc_mode = lib->settings->get_str(lib->settings, "%s.cc_mode",  "Ready", lib->ns);
			...
			if (cid && !id->matches(id, cid))
			{
                ...
                if(strcmp(cc_mode, "Enabled") == 0 || strcmp(cc_mode, "Enforcing") == 0)
                {
                     if (!this->ph1->verify_auth(this->ph1, this->method, message,
                     id_payload->get_encoded(id_payload)))
                     {
                          DBG1(DBG_IKE, "SubjectAlternativeName check failed!");
				id->destroy(id);
				return send_delete(this);
			}
                     else
                     {
                          DBG1(DBG_IKE, "SubjectAlternativeName check success!");
                     }
                }
                else
                {
			...
			/* check do we need to authenticate the peer */
            if(strcmp(cc_mode, "Enabled") != 0 && strcmp(cc_mode, "Enforcing") != 0)
            {
			if (this->auth_peer) 
			{
			...
			{
				DBG1(DBG_IKE, "Skipping peer authentication");
			}
            }
			if (!charon->bus->authorize(charon->bus, FALSE))
			{

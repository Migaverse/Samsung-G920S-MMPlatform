...
openssl_rsa_private_key_t *openssl_rsa_private_key_create(char* engine_id,
							  char* key_name)
{
//#ifndef OPENSSL_NO_ENGINE
...
#ifdef OPENSSL_IS_BORINGSSL
    DBG1(DBG_CFG, "Use EVP_PKEY_from_keystore (RSA): key_name = %s", key_name);
    /*const char *key_id;
    if (sscanf(key_name, "%p", &key_id) != 1) {
    	DBG1(DBG_CFG, "couldn't read private key info");
    	return NULL;
    }
    DBG1(DBG_CFG, "Get RSA EVP_PKEY_from_keystore: key_name = %s, key_id = %s", key_name, key_id);*/
    pkey = EVP_PKEY_from_keystore(key_name);
#else
	eng = ENGINE_by_id(engine_id);
	if (!eng) {
	  DBG1(DBG_CFG, "Cannot get engine");
	  return NULL;
	}
  
	if (!ENGINE_init(eng)) {
	  DBG1(DBG_CFG, "Cannot init engine");
	  ENGINE_free(eng);
	  return NULL;
	}
  
	pkey = ENGINE_load_private_key(eng, key_name, NULL, NULL);
#endif
	if (!pkey) {
	  DBG1(DBG_CFG, "couldn't retrieve private key");
	  ERR_clear_error();
	  ENGINE_free(eng);
	  return NULL;
	}
	DBG1(DBG_CFG, "Successfully retrieve private key");
	
	this = create_empty();
	this->rsa = EVP_PKEY_get1_RSA(pkey);
	this->engine = TRUE;

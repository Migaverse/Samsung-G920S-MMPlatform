...
#include <crypto/conf/conf.c>
#endif

#define MDPP_PROPERTY_NAME "security.mdpp"
#define MDPP_PROPERTY_ENABLED_VALUE "Enabled"
...
{
	char buf[32];
       //If in Android seed the random number generator
       //from /dev/random
#ifdef ANDROID

       const char *RANDOM_DEVICE = "/dev/random";
       int random_fd = -1;
       random_fd = open(RANDOM_DEVICE, O_RDONLY);
       if (random_fd == -1)
       {
       	       DBG1(DBG_LIB, "Cannot open %s for seeding RNG", RANDOM_DEVICE);
	       return FALSE;
       }	 
       while (RAND_status() != 1)
       {
               int bytes_read = 0;
	       if ((bytes_read = read(random_fd, buf, sizeof(buf))) <= 0)
	       {
	               DBG1(DBG_LIB, "Failed to read from /dev/random. Cannot seed RNG");
		       close(random_fd);
		       return FALSE;
	       }
	       DBG1(DBG_LIB, "Read %d bytes from /dev/random.", bytes_read);
	       RAND_seed(buf, bytes_read);
       }
       close(random_fd);
...
plugin_t *openssl_plugin_create()
{
	...
	if (!fips_mode)
	{
		...
		int property_length = __system_property_get(MDPP_PROPERTY_NAME, property_value);
		if (property_length > 0 &&
			!strncmp(MDPP_PROPERTY_ENABLED_VALUE, property_value, PROP_VALUE_MAX))
		{
			DBG4(DBG_LIB, "openssl FIPS mode property set", fips_mode);
			fips_mode = 1;
		}
	}
...
#ifdef OPENSSL_IS_BORINGSSL
	openssl_conf = NCONF_new(NULL);
	DBG1(DBG_LIB, "NCONF_new");
#else
	OPENSSL_config(NULL);
#endif

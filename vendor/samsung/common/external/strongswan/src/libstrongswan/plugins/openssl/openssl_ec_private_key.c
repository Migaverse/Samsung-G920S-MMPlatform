...
openssl_ec_private_key_t *openssl_ec_private_key_create(char* engine_id,
							char* key_name)
{
  private_openssl_ec_private_key_t *this;
...
#ifdef OPENSSL_IS_BORINGSSL
    DBG1(DBG_CFG, "Use EVP_PKEY_from_keystore (EC): key_name = %s", key_name);
    /*const char *key_id;
    if (sscanf(key_name, "%p", &key_id) != 1) {
    	DBG1(DBG_CFG, "couldn't read private key info\n");
    	return NULL;
    }
    DBG1(DBG_CFG, "Get EC EVP_PKEY_from_keystore: key_name = %s, key_id = %s", key_name, key_id);*/
  ...
  this->ec = EVP_PKEY_get1_EC_KEY(pkey);
